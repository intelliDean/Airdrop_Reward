...
contract MyERC20 is IERC20 {
    ...
    function totalSupply() external view returns (uint256) {
        return  s_totalSupply;//revDecimals(s_totalSupply);
    }

    function balanceOf(address _user) external view returns (uint256 balance) {
        return   balances[_user];//revDecimals(balances[_user]);
    }
    ...
    function transfer(address _to, uint256 _value) external returns (bool) {
        ...
        balances[msg.sender] = balances[msg.sender] - _value; //doDecimals(_value);
        balances[_to] = balances[_to] + _value;//doDecimals(_value);
    ...
    function transferFrom(address _from, address _to, uint256 _value) external returns (bool) {
        ...
        if (allowances[_from][_to] < _value /*doDecimals(_value)*/) revert INSUFFICIENT_ALLOWANCE_BALANCE();
        if (balances[_from] < allowances[_from][_to]) revert INSUFFICIENT_BALANCE();
        if (s_totalSupply < balances[_from]) revert BALANCE_MORE_THAN_TOTAL_SUPPLY();

        balances[_from] = balances[_from] - _value;//doDecimals(_value);
        allowances[_from][_to] = allowances[_from][_to] - _value; //doDecimals(_value);
    ...
    function approve(address _spender, uint256 _value) external returns (bool) {
        ...
        if (balances[msg.sender] <  _value /*doDecimals(_value)*/) revert INSUFFICIENT_BALANCE();
        allowances[msg.sender][_spender] = _value; //oDecimals(_value);
        emit Approval(msg.sender, _spender, _value);
        return true;
    }

    function allowance(address _owner, address _spender) external view returns (uint256 remaining) {
        return  allowances[_owner][_spender];// revDecimals(allowances[_owner][_spender]);
    }

    function mint(address _account, uint256 _amount) public {
        onlyOwner();
        uint256 tempSupply = s_totalSupply + _amount; //doDecimals(_amount);
        if (tempSupply > MAX_SUPPLY) revert MAXIMUM_TOKEN_SUPPLY_REACHED();

        s_totalSupply = s_totalSupply + _amount; //doDecimals(_amount);
        balances[_account] = balances[_account] + _amount; //doDecimals(_amount);
    }
    ...
    function burn(uint96 _amount) external {
        ...
        balances[msg.sender] = balances[msg.sender] - _amount;//doDecimals(_amount);
        s_totalSupply = s_totalSupply - _amount;//doDecimals(_amount);

        balances[address(0)] = balances[address(0)] + _amount;//doDecimals(_amount);
    }
